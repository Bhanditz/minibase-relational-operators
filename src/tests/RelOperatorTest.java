 ...
 */
class RelOperatorTest extends TestDriver {
   ...
   */
	protected boolean test1() {
		try {
			...
			Predicate[] preds = new Predicate[] {
					new Predicate(AttrOperator.LT, AttrType.FIELDNO, 3,
							AttrType.FLOAT, 65F),
	...
	protected boolean test2() {
		try {
			...
			saveCounts(null);
			Predicate[] preds = new Predicate[] { new Predicate(
					AttrOperator.EQ, AttrType.FIELDNO, 0, AttrType.FIELDNO, 5) };
	...
	/**
	 * SELECT * FROM Groups G INNER JOIN Rides R ON (G.GroupId = R.GroupId)
	 * INNER JOIN Drivers D ON (R.DriverId = D.DriverId) WHERE D.NumSeats <
	 * G.GroupId;
	 */
	protected boolean test3() {
		try {
			...
			Predicate[] preds = new Predicate[] { new Predicate(
					AttrOperator.EQ, AttrType.FIELDNO, 0, AttrType.FIELDNO, 3) };
			SimpleJoin join1 = new SimpleJoin(new FileScan(s_groups, groups),
					new FileScan(s_rides, rides), preds);

			preds = new Predicate[] { new Predicate(AttrOperator.EQ,
					AttrType.FIELDNO, 2, AttrType.FIELDNO, 6) };
			SimpleJoin join2 = new SimpleJoin(join1, new FileScan(s_drivers,
					drivers), preds);
